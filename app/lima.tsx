...
export default function AkrepZekaScreen() {
  ...
  const chatMutation = trpc.ai.chat.useMutation({
    onError: (err) => {
      console.error("tRPC Chat Error:", err);
      addAkrepMessage("Bağlantımda bir parazit oluştu. Tekrar dener misin? 🦂", true);
    },
  });

  const addAkrepMessage = useCallback(
  ...
  }, [lastUserMessage, isTyping, sendMessage]);
  ...
  const renderMessage = ({ item }: { item: Message }) => (
    <View
      style={[
        ...
        item.from === "user" ? styles.userWrapper : styles.akrepWrapper,
      ]}
    >
      {item.from === "akrep" && (
      ...
      <View
        style={[
          styles.bubble,
          item.from === "user" ? styles.userBubble : styles.akrepBubble,
      ...
      >
        <Text
          style={[
            styles.bubbleText,
            item.from === "user" ? styles.userText : styles.akrepText,
          ]}
        >
          {item.text}
        </Text>
            ...
            <View style={styles.headerInfo}>
              ...
              <View style={styles.statusRow}>
                <View
                  style={[
                    styles.onlineDot,
                    isTyping && { backgroundColor: "#FFD700" },
                  ]}
                />
...
const styles = StyleSheet.create({
  ...
  retryText: { color: "#EF4444", fontSize: 13, fontWeight: "700" },
  inputPanel: {
    padding: 16,
